...
string WcharToUtf8(const wchar_t *pwStr)  
{  
	string strRet;
    if (pwStr == NULL) return strRet;
  
    int len = WideCharToMultiByte(CP_UTF8, 0, pwStr, -1, NULL, 0, NULL, NULL);  
	if (len <= 0) return strRet;
    strRet.resize(len); 
    WideCharToMultiByte(CP_UTF8, 0, pwStr, -1, &strRet[0], len, NULL, NULL);

    return strRet;  
}  
  
wstring Utf8ToWchar(const char *pStr)  
{
	wstring strRet;
    if (pStr == NULL) return strRet;
  
    int len = MultiByteToWideChar(CP_UTF8, 0, pStr, -1, NULL, 0);  
    if (len <= 0) return strRet;
    strRet.resize(len);
    MultiByteToWideChar(CP_UTF8, 0, pStr, -1, &strRet[0], len);

    return strRet;  
}

string AnsiToUtf8(const char *str)  
{  
	if(!str) return string();
	
    DWORD dwNum = MultiByteToWideChar (CP_ACP, 0, str, -1, NULL, 0); 
	if(dwNum <= 0) return string();
    wstring strTemp(dwNum, '\0');
    MultiByteToWideChar(CP_ACP, 0, str, -1, &strTemp[0], dwNum);
...
{  
	...
	wstring strTemp = Utf8ToWchar(str);
	
    int len = WideCharToMultiByte(CP_OEMCP, NULL, strTemp.c_str(), -1, NULL, 0, NULL, FALSE); 
    if (len <= 0) return string();
    string strRet(len, '\0');
    WideCharToMultiByte(CP_OEMCP, NULL, strTemp.c_str(), -1, &strRet[0], len, NULL, FALSE);
...
#else   
    if(sqlite3_open_v2(lpDbFlie,&m_db, SQLITE_OPEN_READWRITE, NULL) != SQLITE_OK)  
#endif   
    {  
        return FALSE;  
    }  
...
{  
    ...
    sqlite3_stmt* stmt;    
#ifdef  UNICODE    
    if(sqlite3_prepare16_v2(m_db, WcharToUtf8(lpSql).c_str(), -1, &stmt, NULL) != SQLITE_OK)  
#else   
    if(sqlite3_prepare_v2(m_db, AnsiToUtf8(lpSql).c_str(), -1, &stmt, NULL) != SQLITE_OK)  
#endif   
    {   
        return FALSE;    
    }    
...
{  
    ...
    char *errmsg = NULL;  
#ifdef  UNICODE    
    if(sqlite3_exec(m_db, WcharToUtf8(lpSql).c_str(), pCallBack, NULL, &errmsg) != SQLITE_OK)  
    {   
        return FALSE;  
    }   
#else   
    if(sqlite3_exec(m_db, AnsiToUtf8(lpSql).c_str(), pCallBack, NULL, &errmsg) != SQLITE_OK)  
    {  
        return FALSE;  
    }   
#endif   
...
#else   
    return Utf8ToAnsi((LPCTSTR)sqlite3_column_text(m_pStmt, nCol));  
#endif   
}  
...
{  
#ifdef  UNICODE    
    if(sqlite3_prepare16_v2(m_pSqlite->m_db, WcharToUtf8(lpSql).c_str(), -1, &m_pStmt, NULL) != SQLITE_OK)  
#else   
    if(sqlite3_prepare_v2(m_pSqlite->m_db, AnsiToUtf8(lpSql).c_str(), -1, &m_pStmt, NULL) != SQLITE_OK)  
#endif   
    {   
        return FALSE;    
    }    
